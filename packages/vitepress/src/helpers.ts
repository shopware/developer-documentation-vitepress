...
export const userCentricsHead = (options: { usercentrics: string, hotjar?: string, gtm?: string, rollbar?: string } = {}) => {
    ...
    if (options.gtm) {
        head.push([
            'script',
            {},
            `(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src='https://sst.shopware.com/pmthrpcx.js?st='+i+dl;f.parentNode.insertBefore(j,f);})(window,document,'script','dataLayer','${options.gtm}');`
        ]);
    }

    if (options.hotjar) {
        head.push(transformToEventListener('Hotjar', [
            'script',
            {},
